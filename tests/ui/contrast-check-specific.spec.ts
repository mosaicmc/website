...
test.describe('Specific Button Contrast Verification', () => {
  ...
  const pagesToCheck = [
    ...
    },
    {
      ...
      url: '/resources',
      elements: [
        // Generic interactive element (link or button)
        { name: 'First Interactive Element', selector: 'main a, main button' },
        // Generic heading
        { name: 'Page Heading', selector: 'main h1, main h2' }
      ]
    }
  ];
      ...
      for (const theme of ['light', 'dark'] as const) {
        ...
        for (const el of pageConfig.elements) {
          ...
          const backgroundColor = await locator.evaluate((e) => {
            let current: HTMLElement | null = e as HTMLElement;
            while (current) {
