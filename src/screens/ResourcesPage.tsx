...
const ResourcesPage = () => {
  ...
  const qualityStatValues = useMemo(
    () =>
      qualityStats.map((stat) => {
        const match = stat.number.match(/^(\d+)/);
        if (!match) {
          return { number: null, suffix: '', raw: stat.number };
        }
        const value = Number(match[1]);
        return { number: value, suffix: stat.number.slice(match[1].length), raw: stat.number };
      }),
    [qualityStats]
  );

  const [qualityCounts, setQualityCounts] = useState<number[]>(() =>
    qualityStatValues.map((stat) => stat.number ?? 0)
  );

  useEffect(() => {
    setQualityCounts(qualityStatValues.map((stat) => (prefersReducedMotion ? stat.number ?? 0 : 0)));
  }, [prefersReducedMotion, qualityStatValues]);

  useEffect(() => {
    const target = qualitySectionRef.current;
    if (!target) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !qualityVisible) {
  ...
  }, [qualityVisible, prefersReducedMotion]);

  useEffect(() => {
    if (prefersReducedMotion || !qualityVisible) return;

    qualityStatValues.forEach((stat, index) => {
      if (stat.number === null) return;
      const duration = 1600;
      const steps = 60;
      ...
      const timer = window.setInterval(() => {
        currentStep += 1;
        const currentValue = Math.min(Math.ceil(increment * currentStep), stat.number ?? 0);
  ...
  }, [prefersReducedMotion, qualityVisible, qualityStatValues]);
  ...
  const featuredResources = [
    {
      ...
      iconBg: 'bg-gradient-to-br from-sun to-sun/80',
    ...
    },
    {
      ...
      iconBg: 'bg-gradient-to-br from-earth to-earth/80',
      iconColor: 'text-white',
    },
    {
      key: 'knowledge-base',
      ...
      iconBg: 'bg-gradient-to-br from-leaf to-leaf/80',
      iconColor: 'text-white',
    },
    {
      key: 'faqs',
  ...
  const secondaryResources = [
    ...
    },
    {
      ...
      iconBg: 'bg-gradient-to-br from-ocean to-ocean/80',
      iconColor: 'text-white',
    },
    {
      key: 'brochures',
      ...
      <Section overlay center padding="lg" overlayClassName="from-ocean/10 via-transparent to-sky/10">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="section-badge bg-card/60 border border-border/60 px-6 py-2 text-sm shadow-lg mb-6">
            ...
            <span className="text-foreground font-medium">{t('resources.hero.badge')}</span>
          </div>
            ...
            const card = (
              <div className="flex flex-col h-full rounded-2xl border border-border/60 bg-card/60 shadow-sm overflow-hidden hover:shadow-md hover:border-border transition-all">
                <div className={cn("h-32 bg-gradient-to-br p-5 flex flex-col justify-between", resource.gradient)}>
                  <div className={cn("w-12 h-12 rounded-xl shadow-md flex items-center justify-center", resource.iconBg)}>
                ...
                <div className="p-5 pt-0 mt-auto">
                  <span className="inline-flex items-center text-sm font-medium text-foreground group-hover:translate-x-1 transition-transform">
                    {t('resources.view')}
          ...
          {brochures.map((brochure) => (
            <Card key={brochure.title} className="flex flex-col h-full rounded-2xl border border-border/60 bg-card/60 shadow-sm overflow-hidden">
              ...
              <CardHeader className="p-4 flex-1">
                <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center mb-3 text-white", getAccentColor(brochure.color))}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {qualityStats.map((stat, index) => (
                <div key={index} className="bg-background/60 backdrop-blur rounded-xl p-4 border border-border/50">
                  <div className={`text-2xl font-bold text-${stat.color} mb-1 tabular-nums`}>
      ...
      <Section padding="lg" className="bg-ocean/10 dark:bg-ocean/20 border-t border-ocean/20">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="cta" variant="cta" className="gap-2 w-full sm:w-auto" asChild>
              <a href="tel:1800813205" aria-label={t('resources.cta.aria.callMosaic')}>
                <Phone className="h-5 w-5" />
                {t('resources.cta.call')}
              </a>
            </Button>
            <Button variant="cta-outline" size="cta" className="gap-2 w-full sm:w-auto" asChild>
