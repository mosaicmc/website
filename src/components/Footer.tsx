...
const Footer = () => {
  ...
  return (
    <footer className="relative overflow-hidden bg-white dark:bg-slate-950 text-foreground transition-colors">
      <div className="pointer-events-none absolute inset-0 opacity-60 overflow-hidden hidden xl:block">
        ...
        <div className="absolute inset-y-0 right-0 w-1/2 md:w-1/3 rounded-full bg-sky/15 dark:bg-sky/20 blur-3xl translate-x-1/4" />
        <div className="absolute inset-y-0 left-0 w-2/5 md:w-1/3 rounded-full bg-ocean/15 dark:bg-ocean/25 blur-3xl -translate-x-1/4" />
      </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
                  ...
                  <div>
                    ...
                    <ul className="mt-2 space-y-2 text-sm text-muted-foreground">
                      ...
                      <li>
                        <Link to="/accessibility" className="inline-flex items-center rounded-full border border-border px-3 py-1.5 text-xs text-foreground transition hover:bg-sand/50 hover:text-ocean dark:hover:bg-white/10 dark:hover:text-sky-text focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 focus:ring-offset-background dark:focus:ring-sky">
                          Accessibility
                        </Link>
                      </li>
                    </ul>
                ...
                {!isEmergencyMode && (
                  <>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`${t('footer.aria.review')} (opens in new tab)`}
                      className="group relative overflow-hidden inline-flex items-center gap-2 rounded-2xl border border-ocean bg-ocean px-3 py-2 text-sm font-semibold text-white shadow-sm transition-all duration-300 hover:shadow-2xl hover:scale-105 active:scale-98 hover:bg-sky hover:text-ocean focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 focus:ring-offset-background before:absolute before:inset-0 before:bg-gradient-to-r before:from-white/10 before:to-transparent before:translate-x-[-100%] group-hover:before:translate-x-[100%] before:transition-transform before:duration-700 dark:border-ocean dark:bg-ocean dark:hover:bg-sky dark:hover:text-ocean dark:focus:ring-ocean"
                    >
      ...
      {!isEmergencyMode && <FooterLanguageBar className="[&_.text-muted-foreground]:text-slate-500 [&_.text-muted-foreground]:dark:text-slate-400" />}

      <div className="border-t border-border bg-white/90 dark:bg-slate-950/90 transition-colors">
        <div className="mx-auto max-w-7xl px-6 py-8 sm:py-10 lg:px-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-slate-600 dark:text-slate-300">{t('footer.copyright')} · {new Date().getFullYear()}</p>
          <div className="flex flex-wrap items-center gap-3 text-xs text-slate-600 dark:text-slate-300">
            <Link to="/accessibility" className="hover:text-foreground transition-colors">
              Accessibility
            </Link>
            <span className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 text-slate-700 dark:text-slate-300">
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2zm0 18a8 8 0 1 1 8-8 8 8 0 0 1-8 8zm0-14a1.5 1.5 0 1 0 1.5 1.5A1.5 1.5 0 0 0 12 6zm0 4a1 1 0 0 0-1 1v5a1 1 0 0 0 2 0v-5a1 1 0 0 0-1-1z" />
              </svg>
              WCAG 2.1 AA
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-slate-600 dark:text-slate-300">{t('footer.followUs')}</span>
            <a href="https://au.linkedin.com/company/mosaic-multicultural-connections" target="_blank" rel="noopener noreferrer" aria-label={`${t('footer.aria.linkedin')} (opens in new tab)`} className="inline-flex items-center justify-center min-h-[44px] min-w-[44px] p-2 rounded-full border border-border text-ocean dark:text-sky transition hover:bg-sand/50 dark:hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background">
            ...
            <a href="https://www.instagram.com/mosaicmc/" target="_blank" rel="noopener noreferrer" aria-label={`${t('footer.aria.instagram')} (opens in new tab)`} className="inline-flex items-center justify-center min-h-[44px] min-w-[44px] p-2 rounded-full border border-border text-ocean dark:text-sky transition hover:bg-sand/50 dark:hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background">
            ...
            <a href="https://www.facebook.com/mosaicmulticulturalconnections/" target="_blank" rel="noopener noreferrer" aria-label={`${t('footer.aria.facebook')} (opens in new tab)`} className="inline-flex items-center justify-center min-h-[44px] min-w-[44px] p-2 rounded-full border border-border text-ocean dark:text-sky transition hover:bg-sand/50 dark:hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background">
