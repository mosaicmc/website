...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({
  ...
  const currentLanguage = useMemo(() => {
    ...
    return (
      availableLanguages.find((l) => l.code.toLowerCase() === code) ||
      ALL_LANGUAGES.find((l) => l.code.toLowerCase() === code) ||
      ALL_LANGUAGES[0]
    );
  ...
  const changeLanguage = (langCode: string) => {
    ...
    const resolved = resolveGoogleLanguage(langCode, options).resolved || langCode;
    document.cookie = `googtrans=/en/${resolved}; path=/`;
  ...
  useEffect(() => {
    const code = (selectedLanguage || 'en').toLowerCase();
    const idx = availableLanguages.findIndex((l) => l.code.toLowerCase() === code);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (options && options.length > 0) {
        ...
        setAvailableLanguages(filterSupportedLanguages(ALL_LANGUAGES, options));
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const resolved = resolveGoogleLanguage(selectedLanguage, googleOptions).resolved || selectedLanguage;
    setFallbackUrl(buildGoogleTranslateUrl(resolved));
  }, [selectedLanguage, googleOptions]);

  useEffect(() => {
    if (typeof window === 'undefined' || typeof document === 'undefined') return;
    const storedUi = localStorage.getItem('preferred-language');
    const storedGt = localStorage.getItem('preferred-language-gt');
    const initialLang = getStoredLanguage();
    if (initialLang) {
      setSelectedLanguage(initialLang.toLowerCase());
      setRotationEnabled(initialLang.toLowerCase() === 'en');
    }
    if (storedUi) {
      const ui = storedUi.toLowerCase();
      setSelectedLanguage(ui);
      setRotationEnabled(ui === 'en');
      if (storedGt && storedGt.toLowerCase() === 'en') {
        localStorage.removeItem('preferred-language-gt');
      }
      return;
    }
    ...
    if (cookieLang === 'en') {
      ...
      localStorage.setItem('preferred-language-gt', cookieLang);
    }
  }, []);

  useEffect(() => {
    if (isOpen || !rotationEnabled || availableLanguages.length === 0) return;
    const id = window.setInterval(() => {
      setRotatingIndex((prev) => {
        const next = (prev + 1) % availableLanguages.length;
        return next;
      });
    }, 3000);
    return () => window.clearInterval(id);
  }, [isOpen, rotationEnabled, availableLanguages.length]);
  ...
  const displayLanguage = useMemo(() => {
    ...
    if (availableLanguages.length === 0) return currentLanguage;
    const boundedIndex = Math.min(
      Math.max(rotatingIndex, 0),
      availableLanguages.length - 1
    );
    return availableLanguages[boundedIndex];
  }, [currentLanguage, isOpen, rotationEnabled, rotatingIndex, availableLanguages]);

  return (
    <div className={cn('relative', className)}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-between gap-2 h-10 md:h-10 min-h-[44px] px-4 rounded-full text-sm font-medium backdrop-blur-md bg-white/20 dark:bg-slate-800/30 border border-white/30 dark:border-slate-700/50 text-gray-700 dark:text-gray-300 hover:bg-white/30 dark:hover:bg-slate-800/50 transition-all duration-300 group focus:outline-none focus:ring-2 focus:ring-ocean focus:ring-offset-2 focus:ring-offset-background"
        ...
        <>
          ...
          <div id={menuId} role="menu" className="absolute top-full right-0 mt-2 w-64 backdrop-blur-xl bg-white/95 dark:bg-slate-900/95 rounded-lg shadow-2xl border border-white/30 dark:border-slate-700/50 z-50 overflow-hidden" tabIndex={-1} onKeyDown={onMenuKeyDown}>
            ...
            <div className="px-4 py-2 border-t border-gray-200/30 dark:border-slate-700/30 bg-gray-50/50 dark:bg-slate-800/50">
              ...
              <p className="mt-1 text-[10px] uppercase tracking-wide text-gray-500 dark:text-gray-500 text-center">
              ...
              {fallbackUrl && (
                <div className="mt-2 text-center">
                  <a
                    ...
                    className="text-[11px] text-ocean dark:text-sky hover:underline"
                  >
