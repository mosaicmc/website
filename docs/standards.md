...
- Use Next.js optimizations and keep dependencies minimal.
...
- When changing stack, tooling, or commands, update `README.md`, `AGENTS.md`, and `CLAUDE.md` together.
- Keep architecture paths accurate (App Router in `src/app`, page composition in `src/screens`).
- For copy changes, run `npm run i18n:check` to prevent missing keys and silent overwrites.
- Avoid feature creep in docs: only document systems that actually exist in the repo.
