...
type RouteInfo = { path: string; filePath: string };
...
const TRANSLATIONS_DIR = path.join(ROOT, 'src', 'locales');

function readFile(file: string) {
  return fs.readFileSync(file, 'utf-8');
}

function crawlNextRoutes(dir = APP_DIR): RouteInfo[] {
  ...
  function walk(d: string) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        ...
        routes.push({ path: routePath, filePath: full });
      }
    }
  }
  walk(dir);
...
function loadTranslations(): TranslationsMap {
  ...
  for (const e of entries) {
    if (e.isDirectory()) {
      const code = e.name;
      const file = path.join(TRANSLATIONS_DIR, code, 'translation.json');
      if (!fs.existsSync(file)) continue;
      try {
        const content = JSON.parse(fs.readFileSync(file, 'utf-8')) as TranslationEntry;
        map[code] = content;
      } catch {
        void 0;
      }
...
function buildTags(title: string, body: string): string[] {
  ...
  const synonyms: Record<string, string[]> = {
    ...
    'home care': ['aged care', 'elder care', 'elderly', 'senior', 'seniors'],
    'aged care': ['home care', 'elder care', 'elderly', 'senior', 'seniors'],
    'elder care': ['home care', 'aged care', 'elderly', 'senior', 'seniors'],
